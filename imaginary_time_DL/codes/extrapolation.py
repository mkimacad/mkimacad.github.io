...
TOTAL_CURRICULUM_REPETITIONS = 2; EPOCHS_PER_PHASE = 15000
CONSOLIDATION_EPOCHS_OVERRIDE = 50000; FINAL_POLISH_EPOCHS = 50000
LOG_EVERY_N_EPOCHS = max(EPOCHS_PER_PHASE//3, 2000)

# --- Data-Fitting Curriculum Configuration ---
USE_SEQUENTIAL_TRAINING = True; SEQUENTIAL_INTERVAL_SIZE = 0.5

# --- Physics Finetuning & Constraint Configuration ---
USE_PHYSICS_FINETUNING_CURRICULUM = True; PHYSICS_SHELL_STEP_SIZE = 2.0
N_CONSTRAINT_POINTS_OVERRIDE = 1000; N_EXTRAPOLATION_CONSTRAINT_POINTS = N_CONSTRAINT_POINTS_OVERRIDE
USE_KINK_PENALTY = True; KINK_PENALTY_WEIGHT = 20.0

# --- Training Data Generation Strategy ---
USE_BOUNDARY_SAMPLING = False; BOUNDARY_SAMPLING_FOCUS_RATIO = 0.5; BOUNDARY_SAMPLING_SHELL_WIDTH = 0.1

# --- Physics Point Generation Strategy ---
USE_PHYSICS_BOUNDARY_SAMPLING = False; PHYSICS_BOUNDARY_FOCUS_RATIO = 0.5
PHYSICS_BOUNDARY_SHELL_WIDTH = 0.1; N_PHYSICS_BOUNDARY_POINTS = N_CONSTRAINT_POINTS_OVERRIDE

# --- Kink Penalty Point Generation Strategy ---
FOCUS_KINK_PENALTY_ON_BOUNDARY = False; N_KINK_PENALTY_BOUNDARY_POINTS = N_CONSTRAINT_POINTS_OVERRIDE

# --- Model and Domain Configuration ---
INTERPOLATION_HALF_WIDTH = 1.0; EXTRAPOLATION_HALF_WIDTH = 10.0
MODEL_WIDTH_OVERRIDE = 64; MODEL_DEPTH_OVERRIDE = 6
...
DYNAMIC_WEIGHT_EMA_ALPHA = 0.99; PEAK_LR = 1e-4; GRADIENT_CLIP_VALUE = 1.0
...
# --- THE FIX IS HERE: Removed the conflicting inner JIT decorators ---
...
def generate_initial_target(key,k_fq_scale=0.1,amp_dom_factor=10.,n_freq=6)->Tuple[Callable,Callable,Callable]:
    ...
    _uv=lambda z:jnp.stack([jnp.sum(amps*jnp.cos(ks*z[...,0:1])*jnp.cosh(ks*z[...,1:2]),axis=-1),jnp.sum(-amps*jnp.sin(ks*z[...,0:1])*jnp.sinh(ks*z[...,1:2]),axis=-1)],axis=-1)
    _psi=lambda z:jnp.sum((amps/(ks+1e-8))*jnp.sin(ks*z[...,0:1])*jnp.cosh(ks*z[...,1:2]),axis=-1)
    _dudx=lambda z:jnp.sum(-amps*ks*jnp.sin(ks*z[...,0:1])*jnp.cosh(ks*z[...,1:2]),axis=-1)
    return _uv,_psi,_dudx
@jit
def get_grad_norm_statistic(grads):leaves=tree_util.tree_leaves(grads);norms=[jnp.max(jnp.abs(leaf))for leaf in leaves if jnp.issubdtype(leaf.dtype,jnp.number)];return jnp.max(jnp.stack(norms))if norms else 0.
@partial(jit,static_argnames=['optimizer','loss_names','loss_functions'])
def static_training_step(params,opt_state,optimizer,loss_names:Tuple,loss_functions:Tuple,weights:Dict):(value_and_grad:=jax.value_and_grad(lambda p: (total_loss_fn:=lambda p:{name:fn(p)for name,fn in zip(loss_names,loss_functions)},jnp.sum(jnp.array([weights.get(name,0.)*total_loss_fn(p)[name] for name in loss_names])))[1],has_aux=True))((loss,losses),grads:=value_and_grad(params));updates,new_opt_s=optimizer.update(grads,opt_state,params);params=optax.apply_updates(params,updates);return params,new_opt_s,loss,losses
...
def dynamic_training_step(params,opt_state,dw_state,optimizer,loss_names:Tuple,loss_functions:Tuple,data_loss_names:Tuple,physics_loss_names:Tuple):
    ...
    ref_grad_stat=jnp.mean(jnp.array([get_grad_norm_statistic(grads[name])for name in data_loss_names if name in grads]));ref_grad_stat=jax.lax.stop_gradient(ref_grad_stat)
    ...
    for name in physics_loss_names:
        if name in grads:
            stat=get_grad_norm_statistic(grads[name]);w_raw=ref_grad_stat/(stat+1e-8);w=DYNAMIC_WEIGHT_EMA_ALPHA*dw_state.get(name,1.)+(1-DYNAMIC_WEIGHT_EMA_ALPHA)*w_raw;new_dw_state[name]=w
...
def load_checkpoint(path:str,init_params,init_opt_state,init_dw_state,init_key):
    ...
    try:
        state_dict=from_bytes(template_state_new,loaded_bytes);key=jnp.asarray(state_dict['key'],dtype=jnp.uint32)
        ...
        try:
            ...
            params=from_bytes(init_params,outer_dict['params']);opt_state=from_bytes(init_opt_state,outer_dict['opt_state'])
            dw_state,phase_index=outer_dict['dw_state'],outer_dict['phase_index'];key=jnp.asarray(outer_dict['key'],dtype=jnp.uint32)
            print(f"  -> Checkpoint found (old format). Resuming from phase {phase_index}.");print("  -> Resaving checkpoint in the new, corrected format...")
...
def _generate_circling_curriculum(half_width:float,interval_size:float)->List[List[Tuple[Tuple[float,float],Tuple[float,float]]]]:
    s=interval_size/2.;L_max=int(jnp.floor((half_width/s-1)/2.))
    get_interval=lambda k,s_val:((2*k-1)*s_val,(2*k+1)*s_val)if k>0 else(((2*k+1)*s_val,(2*k-1)*s_val)if k<0 else(-s_val,s_val))
    return[[(get_interval(i,s),get_interval(j,s))for i in range(-L,L+1)for j in range(-L,L+1)if max(abs(i),abs(j))==L]for L in range(L_max+1)]
def _generate_circling_finetune_curriculum(inner_hw:float,outer_hw:float,step_size:float)->List[List[Tuple[Tuple[float,float],Tuple[float,float]]]]:
    s=step_size/2.;L_max=int(jnp.floor((outer_hw/s-1)/2.))
    get_interval=lambda k,s_val:((2*k-1)*s_val,(2*k+1)*s_val)if k>0 else(((2*k+1)*s_val,(2*k-1)*s_val)if k<0 else(-s_val,s_val))
    finetune_shells=[]
    for L in range(L_max+1):
        indices=[(i,j)for i in range(-L,L+1)for j in range(-L,L+1)if max(abs(i),abs(j))==L]
        shell_boxes=[(x_int,y_int)for i,j in indices if(x_int:=get_interval(i,s),y_int:=get_interval(j,s),max(abs(x_int[0]),abs(x_int[1]),abs(y_int[0]),abs(y_int[1])))[2]>inner_hw]
...
def _build_curriculum_plan():
    ...
    for rep in range(TOTAL_CURRICULUM_REPETITIONS):
        for l_idx,shell in enumerate(_generate_circling_curriculum(INTERPOLATION_HALF_WIDTH,SEQUENTIAL_INTERVAL_SIZE)):
            ...
            cumulative_data_boxes.extend(shell);plan.append({'type':'data_consolidate','rep':rep,'shell':l_idx,'cumulative_boxes':list(cumulative_data_boxes),'epochs':consolidation_epochs})
        if USE_PHYSICS_FINETUNING_CURRICULUM:
            ...
            for l_idx,shell in enumerate(_generate_circling_finetune_curriculum(INTERPOLATION_HALF_WIDTH,EXTRAPOLATION_HALF_WIDTH,PHYSICS_SHELL_STEP_SIZE)):
                ...
                cumulative_physics_boxes.extend(shell);plan.append({'type':'phys_consolidate','rep':rep,'shell':l_idx,'cumulative_boxes':list(cumulative_physics_boxes),'data_boxes':cumulative_data_boxes,'epochs':consolidation_epochs})
        plan.append({'type':'final_polish','rep':rep,'data_boxes':cumulative_data_boxes,'epochs':FINAL_POLISH_EPOCHS})
    print(f"Curriculum plan built with {len(plan)} phases.");return plan
def sample_from_shell(key,num_points:int,outer_hw:float,inner_hw:float)->jnp.ndarray:
    ...
    area_ratio=(outer_hw**2-inner_hw**2)/outer_hw**2;buffer_factor=2./max(area_ratio,0.01)
    while points_found<num_points:
        ...
        accepted=candidates[jnp.max(jnp.abs(candidates),axis=1)>inner_hw];accepted_points.append(accepted);points_found+=accepted.shape[0]
...
def sample_with_boundary_focus(key,num_points:int,inner_box_hw:float,outer_box_hw:float,focus_ratio:float):
    key1,key2=random.split(key);num_boundary_pts=int(num_points*focus_ratio);num_interior_pts=num_points-num_boundary_pts
...
def sample_from_box_boundary(key,num_points:int,half_width:float)->jnp.ndarray:
    ...
    x_bot=random.uniform(side_keys[0],(side_counts[0],1),minval=-half_width,maxval=half_width);x_top=random.uniform(side_keys[1],(side_counts[1],1),minval=-half_width,maxval=half_width)
    y_left=random.uniform(side_keys[2],(side_counts[2],1),minval=-half_width,maxval=half_width);y_right=random.uniform(side_keys[3],(side_counts[3],1),minval=-half_width,maxval=half_width)
    all_points=[jnp.concatenate([x_bot,jnp.full_like(x_bot,-half_width)],axis=1),jnp.concatenate([x_top,jnp.full_like(x_top,half_width)],axis=1),jnp.concatenate([jnp.full_like(y_left,-half_width),y_left],axis=1),jnp.concatenate([jnp.full_like(y_right,half_width),y_right],axis=1)]
...
def run_sequential_training_for_config(key, config, target_fns, use_dynamic_weighting, save_dir, group_id, run_name_for_save):
    ...
    if model_type == 'UV': model = DirectUVMLP(**model_args)
    elif model_type == 'U_ONLY': model = UOnlyMLP(**model_args)
    else: model = ScalarPotentialMLP(**model_args)
    ...
    def _sample_from_boxes(k, num_points, boxes):
        ...
        points_per_box=num_points//len(boxes);remaining=num_points%len(boxes)
        samples = []
    ...
    def _create_full_loss_fns(z_train, z_physics, z_kink):
        ...
        if model_type == 'Potential':
            if config.get('USE_UV_DATA_LOSS',False)and z_train.shape[0]>0:loss_fns['uv_data']=lambda p:jnp.mean((vmap(get_uv_from_potential,(None,None,0))(model,p,z_train)-uv_target_fn(z_train))**2);static_weights['uv_data']=UV_DATA_WEIGHT;data_loss_names.append('uv_data')
            if config.get('USE_POTENTIAL_LOSS',False)and z_train.shape[0]>0:loss_fns['psi_data']=lambda p:jnp.mean((vmap(model.apply,(None,0))(p,z_train)-psi_target_fn(z_train))**2);static_weights['psi_data']=POTENTIAL_DATA_WEIGHT;data_loss_names.append('psi_data')
            if config.get('USE_LAPLACE_LOSS',False)and z_physics.shape[0]>0:loss_fns['laplace']=lambda p:jnp.mean(vmap(get_laplacian_and_grad_laplacian_sq_norm,(None,None,0))(model,p,z_physics)[0]**2);static_weights['laplace']=LAPLACE_WEIGHT;physics_loss_names.append('laplace')
            if USE_KINK_PENALTY and kink_points.shape[0]>0:loss_fns['kink_penalty']=lambda p:jnp.mean(vmap(get_laplacian_and_grad_laplacian_sq_norm,(None,None,0))(model,p,kink_points)[1]);static_weights['kink_penalty']=KINK_PENALTY_WEIGHT;physics_loss_names.append('kink_penalty')
        elif model_type == 'UV':
            if z_train.shape[0]>0:loss_fns['uv_data']=lambda p:jnp.mean((vmap(model.apply,(None,0))(p,z_train)-uv_target_fn(z_train))**2);static_weights['uv_data']=UV_DATA_WEIGHT;data_loss_names.append('uv_data')
            if config.get('use_penalty',False)and z_physics.shape[0]>0:loss_fns['cr_penalty']=lambda p:jnp.mean(vmap(get_uv_and_cr_grads,(None,None,0))(model,p,z_physics));static_weights['cr_penalty']=CR_PENALTY_WEIGHT;physics_loss_names.append('cr_penalty')
            if USE_KINK_PENALTY and kink_points.shape[0]>0:loss_fns['kink_penalty']=lambda p:jnp.mean(vmap(get_cr_and_grad_cr_sq_norm,(None,None,0))(model,p,kink_points)[1]);static_weights['kink_penalty']=KINK_PENALTY_WEIGHT;physics_loss_names.append('kink_penalty')
        elif model_type=='U_ONLY':
            if z_train.shape[0]>0:loss_fns['u_data']=lambda p:jnp.mean((vmap(model.apply,(None,0))(p,z_train)-uv_target_fn(z_train)[...,0])**2);static_weights['u_data']=UV_DATA_WEIGHT;data_loss_names.append('u_data')
            if config.get('use_penalty',False)and z_train.shape[0]>0:loss_fns['dudx_data']=lambda p:jnp.mean((vmap(lambda m,p,z:get_u_and_du_dx(m,p,z)[1],(None,None,0))(model,p,z_train)-dudx_target_fn(z_train))**2);static_weights['dudx_data']=U_DERIV_DATA_WEIGHT;data_loss_names.append('dudx_data')
    ...
    def _run_training_loop(num_epochs, p, opt_s, dw_s, loss_defs, log_prefix):
        ...
        if not loss_fns_t: print(f"  {log_prefix} | No active loss functions. Skipping."); return p, opt_s, dw_s, 0
    ...
    for phase_idx, phase_info in enumerate(curriculum_plan):
        ...
        if USE_PHYSICS_BOUNDARY_SAMPLING:
            num_pts=int(N_PHYSICS_BOUNDARY_POINTS*PHYSICS_BOUNDARY_FOCUS_RATIO);z_physics_boundary=sample_from_shell(phys_boundary_key,num_pts,INTERPOLATION_HALF_WIDTH+PHYSICS_BOUNDARY_SHELL_WIDTH,INTERPOLATION_HALF_WIDTH-PHYSICS_BOUNDARY_SHELL_WIDTH)
        ...
        print(log_msg);
        if z_physics.shape[0]>0 and phase_info['type']=='phys_explore':print(f"    -> Physics Explore: Using {z_physics.shape[0]} points from new box.")
        elif z_physics.shape[0]>0:print(f"    -> Standard/Consolidate: Using {z_physics.shape[0]} total physics points.")
...
def generate_info_text(config, weighting_mode, run_params):
    ...
    loss_parts = []
    model_type = config.get('model_type', 'Potential')
    if model_type == 'UV': loss_parts.extend(["UV Direct"] + (["CR Penalty"] if config.get('use_penalty') else []))
    elif model_type == 'U_ONLY': loss_parts.extend(["U-Only Direct"] + (["dU/dX Data"] if config.get('use_penalty') else []))
    else:
...
def plot_and_display_single_result(model, params, run_info, weighting_mode, target_fns, save_dir, group_id, run_params):
    ...
    if isinstance(model, UOnlyMLP): u_pred = vmap(model.apply, (None, 0))(params, z_plot_grid); uv_pred = jnp.stack([u_pred, jnp.zeros_like(u_pred)], axis=-1)
    elif isinstance(model, ScalarPotentialMLP): uv_pred = vmap(get_uv_from_potential, (None, None, 0))(model, params, z_plot_grid)
    else: uv_pred = vmap(model.apply, (None, 0))(params, z_plot_grid)
...
def save_group_results(group_id, group_run_data, target_fns, save_dir):
    ...
    for ax in axs:
        ...
        ax.grid(True, linestyle=':'); ax.legend(fontsize='small', loc='lower left')
...
def setup_save_directory():
    try: from google.colab import drive; drive.mount('/content/drive'); base_dir = '/content/drive/MyDrive/jax_cauchy_extrapolation'
    except ImportError: base_dir = 'jax_cauchy_extrapolation'
    os.makedirs(base_dir, exist_ok=True); print(f"Save directory is: {base_dir}")
    return base_dir
...
def main():
    ...
    for use_dynamic_weighting in weighting_strategies:
        ...
        for task in tasks:
            ...
            if len(group_run_data) > 1: save_group_results(str(group_id), group_run_data, target_fns, SAVE_DIR)
