...
def fixed_interaction_simulation(M, N, g=1.0, reduc=1.0):
    """
    ...
    if N > 0:
        fixed_time = np.pi/(reduc * 4 * g * np.sqrt(N))
    else:
        fixed_time = np.pi/(reduc * 4 * g)  # Use a non-zero time for N=0
...
def variable_interaction_simulation(M, N, g=1.0, reduc=1.0):
    """
    ...
    for k in range(1, M+1):
        ...
        if avg_n > 0:
            # t_k = π/(4g*√n_avg,k)
            variable_time = np.pi / (reduc * 4 * g * np.sqrt(max(avg_n, 1e-6)))
        else:
            # No excitations left, use minimal time
            variable_time = np.pi / (reduc * 4 * g)  # Use a small non-zero time
...
def plot_comparison(entropies1, entropies2, M, N, reduc=1.0):
    """
    Create a plot comparing the two simulation methods.
    """
    plt.figure(figsize=(12, 6))
    plt.plot(range(0, M+1), entropies1, 'bo-', linewidth=2, label="Case 1: Fixed Int Time")
    plt.plot(range(0, M+1), entropies2, 'ro-', linewidth=2, label="Case 2: Variable Int Time")
    plt.xlabel('Number of Atoms Interacted', fontsize=12)
    plt.ylabel('Entanglement Entropy of Field (Subsystem B)', fontsize=12)
    plt.title(f'Comparison of Entanglement Entropy Evolution\nInitial Field State: |{N}⟩, M={M} Atoms, interaction time reduced by {reduc}', fontsize=14)
...
def plot_excitation_comparison(avg_exc1, avg_exc2, M, N, reduc=1.0):
    """
    Create a plot comparing the average excitation evolution between the two methods.
    """
    plt.figure(figsize=(12, 6))
    plt.plot(range(0, M+1), avg_exc1, 'bo-', linewidth=2, label="Case 1: Fixed Int Time")
    plt.plot(range(0, M+1), avg_exc2, 'ro-', linewidth=2, label="Case 2: Variable Int Time")
    plt.xlabel('Number of Atoms Interacted', fontsize=12)
...
# Parameters
M = 3000  # Number of atoms (now M can be > N)
N = 10  # Initial field excitation number
g = 1.0  # Coupling constant
reduc = 10.0 # Interaction time reduction parameter

# Run simulations with the updated methods
print("Running Case 1: Fixed interaction time...")
entropies_case1, avg_exc1 = fixed_interaction_simulation(M, N, g, reduc)

print("Running Case 2: Variable interaction time based on average excitation...")
entropies_case2, avg_exc2 = variable_interaction_simulation(M, N, g, reduc)

# Entropy plot comparisons
plt_comp1 = plot_comparison(entropies_case1, entropies_case2, M, N, reduc)
plt_comp1.savefig('entropy_comparison.png')
plt_comp1.show()

# Plot excitation results
plt_comp2 = plot_excitation_comparison(avg_exc1, avg_exc2, M, N, reduc)
plt_comp2.savefig('excitation_comparison.png')
plt_comp2.show()
