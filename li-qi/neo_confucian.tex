...
\date{January 5, 2026 (v1)}

\begin{abstract}
In contemporary mathematics, specifically within logic and model theory, a theory is defined as a set of axioms or rules representing essential, core truths. A model is a set of objects that satisfy these axioms. We posit that this relationship maps naturally onto the neo-Confucian concepts of li (principle/pattern) and qi (material force/object). This paper translates fundamental results from mathematical logic, such as the Lindstrom, Lowenheim-Skolem, and Tennenbaum theorems, into the vocabulary of li-qi analysis. By doing so, we demonstrate that the One Li, Many Qi debate can be rigorously formalized, revealing that the nature of qi's uniqueness is partially contingent upon whether the universe is viewed as a computable or non-computable system.
\end{abstract}
...
Computationalism &
\parbox[t]{0.65\columnwidth}{Computability enforces uniqueness (example: Tennenbaum, see also \cite{liu17}).} \\
\hline\hline
\end{tabular}
\end{table}
